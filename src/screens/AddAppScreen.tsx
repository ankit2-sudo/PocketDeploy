...
export default function AddAppScreen() {
  ...
  useEffect(() => {
    ...
    const handleLog = (appId: string, line: LogLine) => {
      if (appId === deployAppId) setDeployLogs(prev => [...prev.slice(-500), line]);
    };
  ...
  const handleDeploy = async () => {
    ...
    try {
      const app = await engineClient.createApp(repoUrl.trim(), appName.trim() || 'my-app', branch);
      ...
      {STEP_LABELS.map((label, i) => (
        <React.Fragment key={label}>
          <View className="items-center">
            <View className={`w-8 h-8 rounded-full items-center justify-center ${i <= stepIndex ? 'bg-indigo-500' : 'bg-zinc-800'}`}>
              <Text className={`text-sm font-bold ${i <= stepIndex ? 'text-white' : 'text-zinc-500'}`}>
                {i < stepIndex ? '\u2713' : i + 1}
              </Text>
            </View>
            <Text className={`text-xs mt-1 ${i <= stepIndex ? 'text-white' : 'text-zinc-500'}`}>{label}</Text>
          </View>
          {i < STEP_LABELS.length - 1 && (
            <View className={`flex-1 h-0.5 mx-2 mb-4 ${i < stepIndex ? 'bg-indigo-500' : 'bg-zinc-800'}`} />
          )}
        </React.Fragment>
      ))}
    </View>
  ...
  const renderCommandRow = (label: string, value: string, onChange: (v: string) => void, fieldKey: string) => (
    <View className="mb-3">
      ...
      <View className="flex-row items-center bg-zinc-900 rounded-lg border border-zinc-800 px-3 py-2">
        {editingField === fieldKey ? (
          ...
          <Text className="flex-1 text-green-400 font-mono text-sm">{value || '(none)'}</Text>
        )}
        <TouchableOpacity onPress={() => setEditingField(editingField === fieldKey ? null : fieldKey)} className="ml-2 px-2 py-1">
          <Text className="text-indigo-400 text-xs">{editingField === fieldKey ? 'Done' : 'Edit'}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <View className="flex-1 bg-[#0a0a0a]">
      <View className="flex-row items-center px-4 pt-14 pb-4 border-b border-zinc-800">
        <TouchableOpacity onPress={() => navigation.goBack()} className="mr-3 w-8 h-8 rounded-full bg-zinc-800 items-center justify-center">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
        <ScrollView className="flex-1 px-4 pt-4" keyboardShouldPersistTaps="handled">
          ...
          {step === 'input' && (
            <View>
              <Text className="text-zinc-400 text-sm mb-4">Paste a GitHub repository URL to get started.</Text>
              ...
              <TouchableOpacity
                ...
                className={`rounded-xl py-4 items-center ${scanning || !repoUrl.trim() ? 'bg-zinc-700' : 'bg-indigo-500'}`}
              >
                {scanning ? (
          ...
          {step === 'review' && config && (
            <View>
              <View className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-4">
                <View className="flex-row items-center mb-3">
                  <View className="w-12 h-12 rounded-xl bg-indigo-500/20 items-center justify-center mr-3">
                    <Text className="text-indigo-400 font-bold text-lg">{getProjectIcon(config.projectType)}</Text>
                  </View>
                  <View className="flex-1">
                    <Text className="text-white font-bold text-lg">{getProjectLabel(config.projectType)}</Text>
                    <Text className="text-zinc-400 text-sm">{config.language} project | Confidence: {config.confidence}</Text>
                  </View>
                </View>
                {config.envVarsDetected && (
                  <View className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg px-3 py-2">
                    <Text className="text-yellow-400 text-xs">.env file detected - you may need to set environment variables after deploy</Text>
                  </View>
                )}
              </View>
              ...
              <TouchableOpacity onPress={handleDeploy} className="bg-indigo-500 rounded-xl py-4 items-center mt-4 mb-2">
